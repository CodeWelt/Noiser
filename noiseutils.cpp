...
void NoiseMapBuilderCylinder::Build (wxGauge* gauge)
{
  ...
  gauge->SetValue(0);
  gauge->SetRange(m_destWidth * m_destHeight);
  for (int y = 0; y < m_destHeight; y++) {
    ...
    for (int x = 0; x < m_destWidth; x++) {
      // Update Noiser GUI
      int step = y * m_destWidth + x;
      if (step % 1000 == 0) gauge->SetValue(step);
      else wxYield();
...
void NoiseMapBuilderPlane::Build (wxGauge* gauge)
{
...
void NoiseMapBuilderSphere::Build (wxGauge* gauge)
{
...
void RendererImage::Render (wxGauge* gauge)
{
  if ( m_pSourceNoiseMap == NULL
    || m_pDestImage == NULL
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      // Update Noiser GUI-Gauge
      int step = y * width + x;
      if (step % 100 == 0) gauge->SetValue(step);
      else wxYield();
...
void RendererNormalMap::Render (wxGauge* gauge, wxImage* parentImage)
{
  if ( m_pSourceNoiseMap == NULL
    || m_pDestImage == NULL
    || m_pSourceNoiseMap->GetWidth  () <= 0
  ...
  for (int y = 0; y < height; y++) {
    ...
    for (int x = 0; x < width; x++) {
      ...
      Color pDester = CalcNormalColor (nc, nr, nu, m_bumpHeight);
      parentImage->SetRGB(x, y, pDester.red, pDester.green, pDester.blue);
      *pDest = pDester;
      // Go to the next point.
      ++pSource;
      ++pDest;
